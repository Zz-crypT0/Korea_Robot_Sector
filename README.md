...
이 프로젝트는 단순 종목 분석을 넘어, **데이터 기반 투자 의사결정을 자동화하는 파이프라인**을 설계·구현한 사례입니다. 이를 통해 다음과 같은 역량을 보여줍니다:

- 국내 주식 데이터 API(pykrx, 네이버 금융 등)를 활용한 시계열/재무 데이터 자동 수집 파이프라인 구축
- 로봇 섹터 특성을 반영한 팩터 기반 스코어링 로직 설계 (밸류에이션, 성장성, 수급, 기술적 지표 등)
- GitHub Actions를 활용한 일간 배치 작업 구성 및 HTML 리포트 자동 생성/배포 자동화
- 설정 파일(config) 기반의 유연한 분석 파라미터 관리 및 재사용 가능한 코드 구조 설계
- Jinja2 템플릿을 활용한 인터랙티브 리포트 생성 및 핵심 지표 시각화
- 운영 환경을 고려한 모듈화(수집–분석–리포트 분리)와 커스터마이징 가능 구조 설계


## 🎯 프로젝트 동기 및 나의 역할

국내 로봇 섹터는 정책·산업 트렌드 측면에서 중장기 성장성이 크지만, 관련 종목들이 여러 업종과 섞여 있어 **일관된 기준으로 비교·선별하기 어렵다**는 문제의식에서 프로젝트를 시작했습니다.  
특히, 뉴스/스토리 중심이 아닌 **정량 지표 기반의 팩터 분석**으로 로봇 섹터를 바라보고, 펀드매니저처럼 섹터 전체를 한 번에 조망할 수 있는 툴을 직접 만들어 보고자 했습니다.

이 프로젝트에서 저는 다음과 같은 역할을 수행했습니다.

- 로봇 섹터 정의 및 핵심 종목/ETF 유니버스 선정
- 데이터 수집 모듈(pykrx, 네이버 금융) 설계 및 오류/지연 상황을 고려한 수집 로직 구현
- 밸류에이션, 성장성, 수급, 기술적 지표를 통합한 투자 스코어링 체계 설계
- 종목별·섹터별 인사이트를 한눈에 파악할 수 있는 HTML 리포트 구조/섹션 기획
- GitHub Actions를 이용한 일간 자동 리포트 생성 및 배포 파이프라인 구성
- 사용자가 종목/파라미터를 손쉽게 바꿔볼 수 있도록 config 중심의 구조로 리팩터링

이를 통해 “특정 섹터를 정의 → 데이터 수집 → 지표 설계 → 모델링/스코어링 → 리포트 자동화”에 이르는 **엔드투엔드 데이터 분석 워크플로우**를 스스로 설계하고 운영 가능한 형태로 구현했습니다.
